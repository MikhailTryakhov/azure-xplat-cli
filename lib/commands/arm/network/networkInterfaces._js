...
exports.init = function (cli) {
  ...
  networkInterfaces.command('create [resource-group] [name] [location]')
    ...
    .execute(function(resourceGroup, name, location, options, _) {
      ...
      if(!parameters.ipConfigurations[index]) {
        ...
        parameters.ipConfigurations[index].name = 'default-name';
      }
  ...
  networkInterfaces.command('show [resource-group] [name]')
    ...
    .execute(function(resourceGroup, name, options, _) {
      ...
      if (!networkInterface) {
        cli.output.warn(util.format($('network interface with name "%s" not found in the resource group "%s"'), name, resourceGroup));
      }
  ...
  networkInterfaces.command('list [resource-group]')
    ...
    .execute(function(resourceGroup, options, _) {
      ...
      var progress;
      try {
        if(typeof networkManagementClient.networkInterfaces.listAll != 'function') {
          resourceGroup = cli.interaction.promptIfNotGiven($('resource-group : '), resourceGroup, _);
          progress = cli.interaction.progress($('Getting the network interfaces'));
          networkInterface = networkManagementClient.networkInterfaces.list(resourceGroup,  _);
        } else {
          if(options.resourceGroup) {
            ...
            progress = cli.interaction.progress($('Getting the network interfaces'));
            networkInterface = networkManagementClient.networkInterfaces.listAll(_);
          }
        }
      } finally {
        progress.end();
      ...
      cli.interaction.formatOutput(networkInterface, function(networkInterface) {
        if (networkInterface.length === 0) {
          cli.output.warn($('No network interfaces found'));
        } else {
          cli.output.table(networkInterface, function (row, item) {
