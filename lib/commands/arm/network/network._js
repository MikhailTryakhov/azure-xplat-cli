...
exports.init = function (cli) {
  ...
  appGatewayWafConfig.command('create [resource-group] [gateway-name] [enable] [waf-mode] [rule-set-type] [rule-set-version]')
    ...
    .option('-e, --enable <enable>', util.format($('whether the web application firewall is' +
      '\n     enabled or not' +
      '\n      supported values are [%s]'), constants.bool))
    .option('-m, --waf-mode <waf-mode>', util.format($('web application firewall mode' +
      '\n      supported values are [%s]'), constants.appGateway.waf.modes))
    .option('--rule-set-type <rule-set-type>', util.format($('the type of the web application' +
      '\n     firewall rule set.' +
      '\n      supported values are [%s]'),constants.appGateway.waf.ruleSets))
    .option('--rule-set-version <rule-set-version>', $('the version of the rule set type'))
    .option('-s, --subscription <subscription>', $('the subscription identifier'))
  ...
  var disabledRuleGroups = appGatewayWafConfig.category('disabled-rule-groups')
  ...
  disabledRuleGroups.command('create [resource-group] [gateway-name] [rule-group-name]')
    ...
    .execute(function (resourceGroup, gatewayName, ruleGroupName, options, _) {
      ...
      options.rules = cli.interaction.promptIfNotGiven($('list of disabled rules: '), options.rules, _);

      var networkManagementClient = getNetworkManagementClient(options);
      var appGateway = new AppGateway(cli, networkManagementClient);
      appGateway.addDisabledRuleGroup(resourceGroup, gatewayName, options, _);
    });
