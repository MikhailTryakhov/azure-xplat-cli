...
exports.init = function (cli) {
  ...
  requestRoutingRules.command('create [resource-group] [gateway-name] [name]')
    ...
    .option('-t, --type [type]', $('rule type. Possible values are:' +
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(result.requestRoutingRules, {name: name})) {
      ...
      if(options.addressPoolId) {
        ...
        var referredbackendAddressPool = utils.findFirstCaseIgnore(result.backendAddressPools, {name: options.addressPoolName});
      ...
      if(options.httpSettingsId) {
        if(options.backendHttpSettingsName) {
          cli.output.warn($('--backend-http-settings-name parameter will be ignored because --http-settings-id and --backend-http-settings-name are mutually exclusive'));
        }
        parameters.requestRoutingRules[index].backendHttpSettings.id = options.httpSettingsId;
      } else if(options.backendHttpSettingsName) {
        var referredbackendHttpSettings = utils.findFirstCaseIgnore(result.backendHttpSettingsCollection, {name: options.backendHttpSettingsName});
        if(!referredbackendHttpSettings) {
          throw new Error(util.format($('No backend http settings with name "%s" found'), options.backendHttpSettingsName));
        }
      ...
      if(options.httpListenerId) {
        ...
        var referredhttpListener = utils.findFirstCaseIgnore(result.httpListeners, {name: options.httpListenerName});
      ...
      result.requestRoutingRules.push(parameters.requestRoutingRules[index]);
  ...
  requestRoutingRules.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var childResult = utils.findFirstCaseIgnore(result.requestRoutingRules, {name: name});
      var index = utils.indexOfCaseIgnore(result.requestRoutingRules, {name: name});
      if(!childResult) {
  ...
  requestRoutingRules.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(result.requestRoutingRules, {name: name});
      if (index === -1) {
        throw new Error(util.format($('request routing rules "%s" not found in the gatewayName "%s"'), name, result.name));
      }
      ...
      result.requestRoutingRules.splice(index, 1);
  ...
  requestRoutingRules.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var childResult = utils.findFirstCaseIgnore(result.requestRoutingRules, {name: name});
      if(!childResult) {
        cli.output.warn(util.format($('request routing rules with name "%s" not found in the gatewayName "%s"'), name, result.name));
      }
  ...
  requestRoutingRules.command('list [resource-group] [gateway-name]')
    ...
    .execute(function(resourceGroup, gatewayName, options, _) {
      ...
      var items = result.requestRoutingRules;
