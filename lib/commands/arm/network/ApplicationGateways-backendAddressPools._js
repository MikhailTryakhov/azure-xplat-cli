...
exports.init = function (cli) {
  ...
  backendAddressPools.command('create [resource-group] [gateway-name] [name] [servers]')
    ...
    .execute(function(resourceGroup, gatewayName, name, servers, options, _) {
      ...
      options.servers = cli.interaction.promptIfNotGiven($('backend addresses : '), servers, _);
  ...
  backendAddressPools.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(result.backendAddressPools, {name: name});
      if (index === -1) {
        throw new Error(util.format($('backend address pools "%s" not found in the gatewayName "%s"'), name, result.name));
      }
  ...
  backendAddressPools.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var childResult = utils.findFirstCaseIgnore(result.backendAddressPools, {name: name});
      if(!childResult) {
        cli.output.warn(util.format($('backend address pools with name "%s" not found in the gatewayName "%s"'), name, result.name));
      }
