...
exports.init = function (cli) {
  ...
  virtualNetworkGateways.command('create [resource-group] [name] [location]')
    ...
    .option('-k, --sku-name [sku-name]', $('gateway SKU name. Possible values are:' +
      '\n     \'Basic\', \'HighPerformance\',\'Standard\', and \'UltraPerformance\''))
    ...
    .execute(function(resourceGroup, name, location, options, _) {
      ...
      if(options.defaultSiteId) {
        ...
        if (!idContainergatewayDefaultSite) {
          throw new Error(util.format($('A local gateway with name "%s" not found in the resource group "%s"'), options.defaultSiteName, resourceGroup));
        }
      ...
      if(options.skuName) {
        parameters.sku.name = validation.isIn(options.skuName, ['Basic', 'HighPerformance', 'Standard', 'UltraPerformance'], '--sku-name');
      } else if(useDefaults) {
  ...
  virtualNetworkGateways.command('set [resource-group] [name]')
    ...
    .execute(function(resourceGroup, name, options, _) {
      ...
      if(options.addressPrefixes) {
        if(virtualNetworkGateway && virtualNetworkGateway.vpnClientConfiguration
          && virtualNetworkGateway.vpnClientConfiguration.vpnClientAddressPool
          && virtualNetworkGateway.vpnClientConfiguration.vpnClientAddressPool.addressPrefixes) {
