...
exports.init = function (cli) {
  ...
  sslPolicy.command('create [resource-group] [gateway-name]')
    ...
    .execute(function(resourceGroup, gatewayName, options, _) {
      ...
      if(options.disableSslProtocols) {
        parameters.sslPolicy.disabledSslProtocols = options.disableSslProtocols.split(',').map(function(item) { return validation.isIn(item, ['TLSv1_0', 'TLSv1_1', 'TLSv1_2'], '--disable-ssl-protocols'); });
      }

      result.sslPolicy = parameters.sslPolicy;
      ...
      cli.interaction.formatOutput(result.sslPolicy, generatorUtils.traverse);
    });
  ...
  sslPolicy.command('show [resource-group] [gateway-name]')
    ...
    .execute(function(resourceGroup, gatewayName, options, _) {
      ...
      var childResult = result.sslPolicy;
      if(!childResult) {
        cli.output.warn(util.format($('ssl policy not found in the "%s"'), result.name));
      }
