...
exports.init = function (cli) {
  ...
  frontendPorts.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(result.frontendPorts, {name: name})) {
      ...
      result.frontendPorts.push(parameters.frontendPorts[index]);
  ...
  frontendPorts.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var childResult = utils.findFirstCaseIgnore(result.frontendPorts, {name: name});
      var index = utils.indexOfCaseIgnore(result.frontendPorts, {name: name});
      if(!childResult) {
  ...
  frontendPorts.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(result.frontendPorts, {name: name});
      if (index === -1) {
        throw new Error(util.format($('frontend ports "%s" not found in the gatewayName "%s"'), name, result.name));
      }
      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete frontend ports with name "%s" from "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
      result.frontendPorts.splice(index, 1);
  ...
  frontendPorts.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var childResult = utils.findFirstCaseIgnore(result.frontendPorts, {name: name});
      if(!childResult) {
        cli.output.warn(util.format($('frontend ports with name "%s" not found in the gatewayName "%s"'), name, result.name));
      }
  ...
  frontendPorts.command('list [resource-group] [gateway-name]')
    ...
    .execute(function(resourceGroup, gatewayName, options, _) {
      ...
      var items = result.frontendPorts;
