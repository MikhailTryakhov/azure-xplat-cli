...
__.extend(AppGateways.prototype, {
  ...
  listSsls: function (resourceGroup, appGatewayName, options, _) {
    ...
    self.interaction.formatOutput(items, function (data) {
      if (data.length === 0) {
        ...
        self._listAppGatewaySslCerificates(appGateway);
      }
    });
  },
  ...
  listFrontendIps: function (resourceGroup, appGatewayName, options, _) {
    ...
    self.interaction.formatOutput(appGateway.frontendIPConfigurations, function (data) {
      if (data.length === 0) {
        ...
        self._listAppGatewayFrontendIpConfigs(appGateway);
      }
    });
  },
  ...
  listFrontendPorts: function (resourceGroup, appGatewayName, options, _) {
    ...
    self.interaction.formatOutput(items, function (data) {
      if (data.length === 0) {
        ...
        self._listAppGatewayFrontendPorts(appGateway);
      }
    });
  },
  ...
  listBackendAddressPools: function (resourceGroup, appGatewayName, options, _) {
    ...
    self.interaction.formatOutput(appGateway.backendAddressPools, function (data) {
      if (data.length === 0) {
        ...
        self._listAppGatewayAddressPools(appGateway);
      }
    });
  },
  ...
  addHttpListener: function (resourceGroup, appGatewayName, httpListenerName, options, _) {
    ...
    if (options.hostName) {
      httpListener.hostName = options.hostName;
    }
  ...
  listProbes: function (resourceGroup, appGatewayName, options, _) {
    ...
    self.interaction.formatOutput(appGateway.probes, function (probes) {
      if (probes.length === 0) {
        ...
        self._listAppGatewayProbes(appGateway);
      }
    });
  },
  ...
  createDisabledRuleGroup: function(resourceGroup, appGatewayName, options, _) {
    ...
    if(!wafConfig) {
      throw new Error(util.format($('waf config not found for application gateway with name "%s" in resource group "%s"'), appGatewayName, resourceGroup));
    }
    ...
    progress = self.interaction.progress(util.format($('Adding new disabled rule group to application gateway waf configuration in "%s"'), appGatewayName));
    try {
      result = self.networkManagementClient.applicationGateways.createOrUpdate(resourceGroup, appGatewayName, result, _);
    } finally {
      progress.end();
  ...
  setDisabledRuleGroup: function(resourceGroup, appGatewayName, options, _) {
    ...
    disabledRuleGroup.ruleGroupName = options.ruleGroupName;
    disabledRuleGroup.rules = options.rules ? options.rules.split(',').map(utils.parseInt) : null;

    progress = self.interaction.progress(util.format($('Updating disabled rule group "%s" in application gateway waf configuration in "%s"'), options.ruleGroupName, appGatewayName));
    try {
      result = self.networkManagementClient.applicationGateways.createOrUpdate(resourceGroup, appGatewayName, result, _);
    } finally {
      progress.end();
  ...
  listDisabledRuleGroups: function(resourceGroup, appGatewayName, options, _) {
    ...
    self.interaction.formatOutput(wafConfig.disabledRuleGroups, function (data) {
      if (data.length === 0) {
        self.output.warn(util.format($('No disabled rule groups found for application waygate "%s" in resource group "%s"'), appGatewayName, resourceGroup));
      } else {
        self.output.table(data, function (row, group) {
          row.cell($('Group name'), group.ruleGroupName);
          row.cell($('Disabled rules'), group.rules ? group.rules.join(', ') : 'All');
        });
      }
    });
  },
  ...
  showDisabledRuleGroup: function(resourceGroup, appGatewayName, options, _) {
    ...
    self.interaction.formatOutput(disabledRuleGroup, generatorUtils.traverse);
  },
  ...
  deleteDisabledRuleGroup: function(resourceGroup, appGatewayName, options, _) {
    ...
    progress = self.interaction.progress(util.format($('Deleting disabled rule group "%s" from application gateway waf configuration in "%s"'), options.ruleGroupName, appGatewayName));
    try {
      result = self.networkManagementClient.applicationGateways.createOrUpdate(resourceGroup, appGatewayName, result, _);
    } finally {
      progress.end();
