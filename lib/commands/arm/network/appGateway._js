...
__.extend(AppGateways.prototype, {
  ...
  addWafConfig: function(resourceGroup, appGatewayName, options, _) {
    ...
    if(options.ruleSetType) {
      result.webApplicationFirewallConfiguration.ruleSetType = options.ruleSetType;
    }

    if(options.ruleSetVersion) {
      result.webApplicationFirewallConfiguration.ruleSetVersion = options.ruleSetVersion;
    }
  ...
  addDisabledRuleGroup: function(resourceGroup, appGatewayName, options, _) {
    var self = this;
    var result = self.get(resourceGroup, appGatewayName, _);
    if (!result) {
      throw new Error(util.format($('application gateway with name "%s" not found in the resource group "%s"'), appGatewayName, resourceGroup));
    ...
    if (wafConfig.disabledRuleGroups.filter(function (group) {
      return group.ruleGroupName === options.ruleGroupName;
    }).length) {
    ...
    wafConfig.disabledRuleGroups.push({
      ...
      rules: options.rules // TODO: parse csv into int32 array or null
    });
