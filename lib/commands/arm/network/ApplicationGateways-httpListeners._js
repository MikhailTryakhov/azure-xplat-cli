...
exports.init = function (cli) {
  ...
  httpListeners.command('create [resource-group] [gateway-name] [name]')
    ...
    .option('-o, --frontend-port-id [frontend-port-id]', $('sets frontend port. This option is' +
      '\n     mutually exclusive with --frontend-port-name'))
    .option('-r, --protocol [protocol]', $('protocol. Possible values are: \'Http\'' +
      '\n     and \'Https\''))
    .option('-c, --ssl-cert [ssl-cert]', $('sets ssl certificate. This option is' +
      '\n     mutually exclusive with --ssl-cert-id'))
    .option('-e, --ssl-cert-id [ssl-cert-id]', $('sets ssl certificate. This option is' +
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(result.httpListeners, {name: name})) {
      ...
      if(options.frontendPortId) {
        ...
        var referredfrontendPort = utils.findFirstCaseIgnore(result.frontendPorts, {name: options.frontendPortName});
      ...
      if(options.sslCertId) {
        ...
        var referredsslCertificate = utils.findFirstCaseIgnore(result.sslCertificates, {name: options.sslCert});
      ...
      result.httpListeners.push(parameters.httpListeners[index]);
  ...
  httpListeners.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var childResult = utils.findFirstCaseIgnore(result.httpListeners, {name: name});
      var index = utils.indexOfCaseIgnore(result.httpListeners, {name: name});
      if(!childResult) {
  ...
  httpListeners.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(result.httpListeners, {name: name});
      if (index === -1) {
        throw new Error(util.format($('http listeners "%s" not found in the gatewayName "%s"'), name, result.name));
      }
      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete http listeners with name "%s" from "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
      result.httpListeners.splice(index, 1);
  ...
  httpListeners.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var childResult = utils.findFirstCaseIgnore(result.httpListeners, {name: name});
      if(!childResult) {
        cli.output.warn(util.format($('http listeners with name "%s" not found in the gatewayName "%s"'), name, result.name));
      }
  ...
  httpListeners.command('list [resource-group] [gateway-name]')
    ...
    .execute(function(resourceGroup, gatewayName, options, _) {
      ...
      var items = result.httpListeners;
