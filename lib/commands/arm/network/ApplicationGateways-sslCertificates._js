...
exports.init = function (cli) {
  ...
  sslCertificates.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(result.sslCertificates, {name: name})) {
      ...
      result.sslCertificates.push(parameters.sslCertificates[index]);
  ...
  sslCertificates.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var childResult = utils.findFirstCaseIgnore(result.sslCertificates, {name: name});
      var index = utils.indexOfCaseIgnore(result.sslCertificates, {name: name});
      if(!childResult) {
  ...
  sslCertificates.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(result.sslCertificates, {name: name});
      if (index === -1) {
        throw new Error(util.format($('ssl certificates "%s" not found in the gatewayName "%s"'), name, result.name));
      }
      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete ssl certificates with name "%s" from "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
      result.sslCertificates.splice(index, 1);
  ...
  sslCertificates.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var childResult = utils.findFirstCaseIgnore(result.sslCertificates, {name: name});
      if(!childResult) {
        cli.output.warn(util.format($('ssl certificates with name "%s" not found in the gatewayName "%s"'), name, result.name));
      }
  ...
  sslCertificates.command('list [resource-group] [gateway-name]')
    ...
    .execute(function(resourceGroup, gatewayName, options, _) {
      ...
      var items = result.sslCertificates;
