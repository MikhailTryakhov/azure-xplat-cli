...
exports.safeGetResource = function(client, resourceType, resourceGroup, name, description, _) {
  var progress = cli.interaction.progress(util.format($('Looking up the ' + description + ' "%s"'), name));
  var getResult;
  try {
    getResult = client[resourceType].get(resourceGroup, name, null, _);
  } catch (e) {
    if (e.statusCode === 404) {
      getResult = null;
    } else {
